# safestringlib as Used by FieldComm Group Tooling
This library is an adapted from the Intel safestringlib project on GitHub, which is described below.  safestringlib is released under the MIT license.

Scroll down in this README to see a listing of the C functions provided in the library.

The changes to the library are:
- g++ command line option "-fstack-protector-strong" was removed from the makefile to allow builds with older compiler versions.
- added a simple implementation of sprintf_s  that protects against buffer overflows.
